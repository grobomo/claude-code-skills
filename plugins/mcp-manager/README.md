...
Setup runs automatically on first skill load. Then:
1. Type `/mcp` -> select **mcp-manager** -> **Connect**
2. Ask Claude: *"show me my MCP servers"*
...
Just talk to Claude naturally. Claude handles all the tool calls behind the scenes.

| You say | What happens |
|---------|-------------|
| *"show me my MCP servers"* | Lists all servers and their status |
| *"start the wiki server"* | Spawns the process or connects HTTP |
| *"stop the wiki server"* | Stops a running server |
| *"add a new MCP server for my python script at ~/tools/server.py"* | Registers it in servers.yaml |
| *"what tools does the wiki server have?"* | Lists available tools |
| *"search my servers for anything related to docs"* | Searches servers + tools by keyword |
| *"reload my MCP config"* | Re-reads servers.yaml without restart |
| *"scan for any MCP servers I haven't registered"* | Discovers unregistered servers nearby |
| *"how much memory are my MCP servers using?"* | System health + memory stats |

**To reload after code changes:** `/mcp` --> select mcp-manager --> Reconnect. This restarts the mcpm process with the latest build.
...
Setup automatically:
- Backs up your existing `settings.json`, `CLAUDE.md`, and `.mcp.json`
- Adds mcpm to your Claude Code settings
- Creates behavioral instructions (or injects rules into CLAUDE.md if instruction-manager isn't installed)
- Stores backups in `~/.claude/backups/mcp-manager/`

## Uninstall

Ask Claude: *"uninstall mcp-manager"*

Or manually:
```bash
node ~/.claude/skills/mcp-manager/uninstall.js
claude plugin uninstall mcp-manager
```

Uninstall restores all files from backup and removes everything setup created.

---

## Standalone or Full Stack

mcp-manager works on its own. For the full experience -- secure credential storage, auto-prompt injection, and tool auto-detection -- install **super-manager**:
